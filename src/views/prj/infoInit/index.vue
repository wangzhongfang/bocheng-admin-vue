<template>
  ...
  <ContentWrap>
    ...
    >
      ...
      <el-table-column label="期初资金（￥）" align="center" prop="fundsAmount">
        <template #default="scope">
          <el-link type="success" @click="openForm('update', scope.row.id)">
            {{
              scope.row.fundsDirection === 0
                ? '-' + (scope.row.fundsAmount || '--')
                : scope.row.fundsAmount || '--'
            }}</el-link
          >
        </template>
      </el-table-column>
      <el-table-column label="材料总金额（￥）" align="center" prop="prjNo">
        <template #default="scope">
          <span>{{ scope.row.prjNo || '--' }}</span>
        </template>
      </el-table-column>
      <el-table-column label="操作" align="center" min-width="120px">
        <template #default="scope">
          <el-button
            link
            type="primary"
            @click="openInfoInitForm('update', scope.row.id)"
          ...
          <el-button
            link
            type="primary"
            @click="openInventoryForm('update', scope.row.id)"
            v-hasPermi="['prj:info:update']"
          >
            库存期初
          </el-button>
  ...
  <!-- 期初资金表单弹窗：添加/修改 -->
  <FoundsForm ref="foundsFormRef" @success="getList" />

  <!-- 库存材料表单弹窗：添加/修改 -->
  <InventoryForm ref="inventoryFormRef" @success="getList" />
</template>
...
/** 添加/修改操作 */
const foundsFormRef = ref()
const openInfoInitForm = (type: string, id?: number) => {
  foundsFormRef.value.open(type, id)
}

const inventoryFormRef = ref()
const openInventoryForm = (type: string, id?: number) => {
  inventoryFormRef.value.open(type, id)
}
